...
Future post(dynamic url, {Map<String,String> headers, body, Encoding encoding}) async {
  return _processResponse(
      await _withClient((client) => client.post(url,
          headers: headers, body: body, encoding: encoding)));

}

_processResponse(http.Response response) {
  _logger.fine("${response.request.method} ${response.request.url}: ${response.body}");
