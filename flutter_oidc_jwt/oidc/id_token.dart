...
import 'package:adventist_student/dartjwt/dart_jwt.dart' hide OpenIdJwtClaimSet;
...
class IdToken extends JsonWebSignature<MapJwtClaimSet> {
  ...
  IdToken._internal(JwsHeader header, MapJwtClaimSet claimSet,
      JwsSignature signature, String signingInput)
      : openIdClaimsSet = new OpenIdJwtClaimSet.fromJson(claimSet.json),
        idTokenValue = signingInput,
  ...
  factory IdToken.decode(String jwtToken) {
    ...
    var jsonValues = Base64EncodedJson.decodeToJson(base64Segs.elementAt(1));

    final claimSet = new MapJwtClaimSet.fromJson(jsonValues);
...
//    final claimSet = new MapJwtClaimSet.fromJson(new Map.unmodifiable(json));
  ...
  factory IdToken.decodeOriginal(String jwtToken) {
    final base64Segs = jwtToken.split('.');
    if (base64Segs.length != 3)
      throw new ArgumentError(
          "JWS tokens must be in form '<header>.<payload>.<signature>'.\n"
...
class IdTokenValidationContext extends JwtClaimSetValidationContext {
  ...
  IdTokenValidationContext(
      ...
      this.clientId,
      this.nonce,
      Duration expiryTolerance: const Duration(seconds: 30),
      this.validateIssuerAndAudience: true})
